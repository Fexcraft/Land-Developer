...
public class County implements Saveable, Layer, LDUIModule {
	...
	public void on_interact(BaseCon container, ModuleRequest req){
		...
		switch(req.event()){
			...
			case "seat": if(seat >= 0) container.open(LDKeys.MUNICIPALITY, 0, seat, 0);break;
			case "region": container.open(LDKeys.REGION, 0, region.id, 0); break;
			case "citizen": container.open(UI_CITIZEN_LIST); break;
			case "municipalities": container.open(UI_MUNICIPALITY_LIST); break;
			...
			case "municipality.goto":{
				...
				if(!municipalities.contains(mun)) break;
			...
			case "municipality.remove":{
				...
				muni.save();
				save();
				Broadcaster.announce(channel_member, "county.municipality.removed", muni.name(), name(), id);
				Broadcaster.announce(muni.channel_member, "county.municipality.removed", muni.name(), name(), id);
				container.open(UI_MUNICIPALITY_LIST);
				break;
			}
			case "citizen.invite":{
			...
			case "citizen.remove":{
				...
				if(cit != null && !manage.isManager(cit.uuid)){
					...
					this.mail.mails.add(mail);
					Broadcaster.announce(channel_member, "county.citizen.removed", cit.getPlayerName(), name(), id);
				}
				container.open(UI_CITIZEN_LIST);
				break;
			}
			...
			case "staff.remove":{
				...
				if(staff != null && !manage.isManager(staff)){
					...
					Broadcaster.announce(channel_member, "county.staff.removed", staff.getPlayerName(), name(), id);
				}
			...
			case "staff.setmanager":{
				...
				if(staff != null){
					...
					Broadcaster.announce(channel_member, "county.manager_set", staff.getPlayerName(), name(), id);
				}
				container.open(UI_MAIN);
				break;
			}
